...
12. **Apply `deslopify` to every comment body and to the review body.** This step is mandatory before submitting. The skill removes AI writing tells (filler phrases, hedging, false enthusiasm, em dashes, bulleted-everything). Keep the severity prefix and structure; rewrite the prose. Apply to both inline comment bodies and the top-level review body.

13. **Decide the verdict.**
...
16. **Submit the review** as one atomic GitHub review:
    ```
    gh api -X POST repos/<owner>/<repo>/pulls/<N>/reviews \
      -f event=<APPROVE|REQUEST_CHANGES|COMMENT> \
      -f body="<deslopified review body>" \
...
- **Never approve a PR whose quality gates fail locally** — that's a P0 finding, verdict is `REQUEST_CHANGES`.
