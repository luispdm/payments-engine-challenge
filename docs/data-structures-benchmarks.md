...
Single-threaded comparison of two tx ledger storage layouts. Concurrency
is the next bench (separate report); this one isolates the structural
cost without contention confound.
...
suite in `tests/scenarios.rs` runs every CSV scenario through both
engines and asserts byte-for-byte equivalence.
...
- **Concurrency is a separate question.** Under a sharded lock (DashMap)
  ...
  holding the storage variant fixed at v1's layout.
